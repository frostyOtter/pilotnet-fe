...
interface SynchronizedCombinationVideoPlayerProps {
  ...
  onUpdate: (data: CombinationPredictionData) => void;
...
const SynchronizedCombinationVideoPlayer: React.FC<SynchronizedCombinationVideoPlayerProps> = ({ 
  videoUrl, 
  websocket,
  onUpdate,
  isInitialized,
  ...
  const [predictions, setPredictions] = useState<CombinationPredictionData[]>([]);
  ...
  useEffect(() => {
    ...
    const handleWebSocketMessage = (event: MessageEvent) => {
      ...
      if (data.status === 'error') {
        console.error('Combined demo error:', data.message);
        return;
      }
      ...
      if (data.status === 'streaming' || data.status === 'initialized') {
        setPredictions(prev => [...prev, data as CombinationPredictionData]);
      }
    };
...
export default SynchronizedCombinationVideoPlayer;